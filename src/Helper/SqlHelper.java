...
public class SqlHelper {
	public static JSONArray getStations(){
		...
		try {
			...
			//G1.stationid,G1.stationname,G2.x,G2.y from G_STATIONINFO G1 inner join G_STATIONORIENT G2 on G1.stationid=G2.stationid
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()){
				JSONObject jsonObject = new JSONObject();
				//JSONObject stations1 = new JSONObject();
				jsonObject.put("stationid", rs.getString("stationid"));
				jsonObject.put("stationname", rs.getString("stationname"));
				//jsonObject.put("point", jsonObject.put("lng", rs.getDouble("x")));
				jsonObject.put("lng", rs.getFloat("x"));
				jsonObject.put("lat", rs.getFloat("y"));
				//jsonObject.put("point",jsonObject1);
